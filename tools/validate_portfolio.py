...
EXPECTED_COUNTS={'control_plane':1,'foundational':7,'ai_native':8}
...
def validate(root:Path=ROOT)->dict:
 ...
 errors=[]
 if len(active)!=16 or data.get('active_repository_count')!=16: errors.append('active repository count must be 16')
 ...
 if len(set(names))!=16: errors.append('active names must be unique')
 ...
 cats={c:sum(x['category']==c for x in active) for c in EXPECTED_COUNTS}
 if cats!=EXPECTED_COUNTS: errors.append(f'invalid category counts: {cats}')
 for x in active:
 ...
 if {r['repository'] for r in rows}!=set(names): errors.append('CSV ledger must cover exactly 16 active repositories')
